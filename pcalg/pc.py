...
from pcalg.graph import visualize_graph, grapher
...
def demo_pc_algorithm():
    ...
    data_path = Path(__file__).resolve().parent.parent / "test_data.csv"
    ...
    # 7. 可视化
    print("\n[7] 可视化因果图...")
    demo_plot = Path(__file__).resolve().parent.parent / "demo" / "causal_graph_demo.png"
    demo_plot.parent.mkdir(parents=True, exist_ok=True)
    visualize_graph(graph, label_dict, save_path=str(demo_plot))
